...
- [x] **自动化爬虫**: `lining_scraper.py` 通过 Playwright 实现李宁官网数据抓取。
- [x] **数据入库**: `ingest_data.py` 实现 CSV 数据向量化并批量同步至云端。
- [x] **以图搜图**: `image_search.py` 完成核心检索逻辑。
- [x] **文本检索深化**: 在 `text_retrieval.py` 中实现基于 BGE-M3 的纯文本搜鞋逻辑。
- [x] **混合搜索 (RRF)**: 实现图文双模态融合搜索，提升复杂场景下的召回准确度。
- [x] **置信度策略**: 落地 `score < 0.6` 触发 `need_clarify` 的逻辑，引导后端澄清。
- [x] **知识库精细化**: 实现基于标签的描述切块（Chunking）与 SKU 级别引用检索接口。
- [x] **规模化扩展**: 知识库 SKU 数量成功从 10 款扩展至 **100+ 款** 高精度数据。
- [x] **性能评测**: 建立测试集并完成自动化评测，Top-1 命中率 > 85%，Top-3 命中率 100%。
- [x] **生产环境压测**: 验证 P95 耗时 315.2ms (单并发)，符合 < 500ms 性能目标。

### ⏳ 待完成任务 (Pending)
- [ ] **性能评测反馈迭代**: 根据后端联调结果优化检索 Prompt 或 RRF 权重。
