...
- [x] **自动化爬虫**: `lining_scraper.py` 通过 Playwright 实现李宁官网数据抓取、图片下载及 JPG 转换。
- [x] **数据入库**: `ingest_data.py` 实现 CSV 数据向量化并批量同步至云端（首批 10 款商品已入库）。
- [x] **以图搜图**: `image_search.py` 完成核心检索逻辑，并修复了 Qdrant API 兼容性问题。
- [x] **功能验证**: `test_rag_image_search.py` 成功跑通 WebP/JPG 图片的端到端检索测试。

### ⏳ 待完成任务 (Pending)
- [ ] **文本检索深化**: 在 `text_retrieval.py` 中实现基于 BGE-M3 的纯文本搜鞋逻辑。
- [ ] **混合搜索 (RRF)**: 实现图文双模态融合搜索，提升复杂场景下的召回准确度。
- [ ] **知识库精细化**: 针对商品详情页的文本进行长文本切块（Chunking），提高引用片段（Citations）的精准度。
- [ ] **置信度策略**: 在代码层面落地 `score < 0.6` 触发 `need_clarify` 的逻辑。
- [ ] **规模化扩展**: 扩充知识库 SKU 数量（从 10 款扩展至 100+ 款）。
